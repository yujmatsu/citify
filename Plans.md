...
- [x] エージェント間 Pub/Sub メッセージング — `pkg/pubsub.py` (Publisher/Subscriber 抽象 + envelope) + Terraform topics/subscriptions/DLQ/IAM + scrapers publish + translator worker。54 unit tests PASSED。A-4 → A-5 → A-6 パイプラインのスケルトン完成。残: terraform apply + 実 Pub/Sub での live test (要 GCP 接続)
