...
- **日付**: 2026-06-15
- **ステータス**: 実装済み（本番 smoke を経て Lv3→**Lv2.5 プランナー主導**に着地）
- **関連**: docs/PROJECT.md §審査基準①、docs/plans/2026-06-07-agent-autonomy-v2-design.md（P3 マルチエージェント）

> **実装後追記（本番 smoke の知見）**: pure Lv3（Coordinator が AgentTool 経由で専門家を逐次采配）は本番で
> 2つの問題が出た — (1) gemini-2.5-flash が専門家を1人で済ませがちで**カバレッジが薄く不安定**、
> (2) 逐次ディスパッチ＋自己検証の積み増しで**244s〜>270s と 300s 壁に接触**。
> このため最終形は **Lv2.5「プランナー主導＋並列実行」** に変更:
> **プランナー LLM が record_plan で調査計画と対象専門家を決定（計画・采配は LLM が握る＝可視）→
> コードが専門家を並列実行（高速）→ 統合 → 自己検証（必須1回）**。
> カバレッジ床（population/living_safety/topics）で核を保証。crew は flag フォールバックとして温存。
> 以下 §3〜§5 の「Coordinator が制御フロー所有」は当初案。現行コードは上記 Lv2.5。
