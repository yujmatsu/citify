# Phase F v0.3 — Reinfolib 統合仕様書 (45 自治体 × 8 API 縮小版)

> 国土交通省「不動産情報ライブラリ」(reinfolib.mlit.go.jp) の **8 個の Web API** を統合し、Phase A `press_rss publish-all` で動作確認済の **45 自治体**の街ダッシュボードに「住居・防災・子育て・医療・教育・移住」軸の客観統計を追加する。

- **Owner**: Yuji
- **Status**: Draft v0.3 (v0.2 Reviewer 指摘 Critical 1 + 高 2 + 中 6 + 低 2 を全反映、GIS 3 本を v3 降格)
- **作成日**: 2026-05-27
- **位置付け**: Phase D MVP の延長、Plan A の客観統計補強 (Should、AI Agent 要素なし)
- **前バージョン**: v0.1 (`PHASE_F_REINFOLIB.md`、2 API)、v0.2 (`PHASE_F_REINFOLIB_v0.2.md`、11 API、Critical fact error)
...
### 対象自治体 (Reviewer v0.2 #1 Critical 対応)

**45 自治体 — Phase A `publish-all` で動作確認済の press_rss feed 元自治体**

抽出ロジック:
```sql
-- 動作確認済 = publish-all の log で publish_all.ok かつ n > 0
SELECT DISTINCT municipality_code
FROM tier1_supplements.csv
WHERE municipality_code IN (
  '01000', '02000', '04000', '07000', '08000', '09000', '10000', '11000',  -- 北/東北
  '12000', '14000', '15000', '17000', '19000', '22000', '23000', '32000',  -- 中部/西
  '33000', '34000', '40000', '43000', '46000',                              -- 西
  '01100', '01213', '12203', '12217', '14130', '14150', '15100', '20202', '22203',
  '24202', '26100', '27100', '27140', '29201', '33100', '34100', '38201', '40100',
  '40130', '42202', '43100', '44201'
);
```

= **都道府県 21 + 政令市 12 + 中核市 12 = 45 自治体** (Phase A publish-all 動作確認済、tier1_supplements.csv より)

**前提タスク (Phase F 着手前)**:
- [ ] tier1_supplements.csv の 45 自治体コードを Phase F の対象として `apps/api/scripts/load_reinfolib_stats.py` の入力リストに固定
...
- [x] 仕様書 v0.3 が作成された
- [ ] 仕様書 v0.3 が Reviewer subagent で再レビューされる (Critical/High 指摘がなければ承認)
- [ ] ユーザー承認
- [ ] 政令市区単位 sample call 結果が v0.3 改訂履歴に記録される
