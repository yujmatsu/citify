...
2. **score_age (年代適合性、0-25)**
   ...
   - 30-39 = 子育て世代 (育児・住宅ローン・税控除・教育費初期)
   - 40-49 = 働き盛り・親介護期 (教育費中盤・介護・税制改正・キャリア後期)
   - 50+ = シニア・退職前後 (年金・医療・介護・防災・住み替え)
...
def build_multi_user_prompt(
    *,
    ...
    """Phase Y: N ペルソナを 1 prompt で一括評価する user prompt 構築。

    Args:
        personas: 各 persona の dict (user_id, age_group, interests, municipality_codes)
    """
    if translated_summary:
        summary_block = "\n".join(f"- {line}" for line in translated_summary)
        content_section = f"""# 評価対象 (A-5 翻訳サマリ、優先)
...
{content_text[:2000]}{"..." if len(content_text) > 2000 else ""}"""
    ...
    for i, p in enumerate(personas, start=1):
        ...
        munis = p.get("municipality_codes") or []
        interests_str = ", ".join(interests) if interests else "(なし)"  # type: ignore[arg-type]
        munis_str = ", ".join(munis) if munis else "(なし)"  # type: ignore[arg-type]
        personas_block_lines.append(
            f"## ペルソナ {i}\n"
            f"- user_id: {p.get('user_id')}\n"
            f"- 年代: {p.get('age_group')}\n"
            f"- 関心軸: {interests_str}\n"
            f"- 登録自治体: {munis_str}"
        )
    personas_block = "\n\n".join(personas_block_lines)

    return f"""以下の発言について、{len(personas)} 個のペルソナそれぞれの関連性を 4 軸で評価してください。
...
# ペルソナ群 ({len(personas)} 件)

{personas_block}

# タスク
各ペルソナについて、4 軸 (topic/age/geographic/urgency、各 0-25) でスコアリングし、
合計を relevance_score (0-100) に格納してください。
ペルソナ関心軸のうち合致したものを matched_interests に列挙してください。
reasoning は 200 字以内で簡潔に、倫理ルールを必ず遵守してください。

出力は results 配列に {len(personas)} 個のペルソナ評価結果を、入力順に格納してください。
各要素は user_id (入力の user_id をそのまま) + 4 軸スコア + reasoning を持ちます。
"""
