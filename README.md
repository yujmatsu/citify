...
| Week | 期間 | 進捗 |
|---|---|---|
| **Week 0** | 5/19-5/25 | ✅ **完了** — GCP 基盤 + 自治体マスタ 1,796 件 + 国会 API 検証 + DiscussNet/voices_asp 構造調査 |
| Week 1 | 5/26-6/1 | 🔜 着手予定 (Terraform apply + 国会 API クライアント + RAG) |

詳細な進捗は [Plans.md](./Plans.md)、タスク一覧は [tasks.json](./tasks.json) 参照。

### Week 0 サンプル Cloud Run デプロイ済

| 項目 | 値 |
|---|---|
| **サービス URL** | https://hello-citify-46070204654.asia-northeast1.run.app |
| **Smoke test** | `curl /` → HTTP 200 / ~360ms |
| **イメージ** | `gcr.io/cloudrun/hello` (Week 1 で `citify-api` に置き換え予定) |
| **GCP プロジェクト** | `citify-dev` (リージョン: `asia-northeast1`、予算アラート ¥7,500/月) |
| **有効化 API** | 14 個 (依存込み 23 個) |

スクレイパー分類は **5 ベンダ** に拡張済 (kaigiroku / voices_asp / db_search / kensakusystem_legacy / custom)。詳細は [docs/DATA_SOURCES.md](./docs/DATA_SOURCES.md)。
...
> 14 個指定すると依存込みで合計 23 個が enabled になる。Week 0 で検証済 (`citify-dev` プロジェクト)。
...
│   └── distributor/
│
├── scrapers/           # データ収集パーサー (5 ベンダ + 国会)
│   ├── kokkai/             # 国会会議録 API (JSON)
│   ├── kaigiroku_net/      # DiscussNet SPA (Playwright + Chromium)
│   ├── voices_asp/         # VOICES/Web (BeautifulSoup + Shift_JIS)
│   ├── db_search/          # DB-Search 系 (*.dbsr.jp)
│   ├── kensakusystem/      # kensakusystem.jp 旧 HTML4 (legacy)
│   ├── custom/             # 独自実装 (北区 Discuss Cabinet 等)
│   ├── press_rss/          # 47都道府県 + 政令市プレスリリース
│   └── ...
│
├── infra/              # Terraform IaC
│   ├── modules/
